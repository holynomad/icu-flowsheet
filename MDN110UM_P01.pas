{===============================================================================
   Program ID    : MDN110UM_P01
   Program Name  : ICU 통합 간호기록지 자동서식스캔 양식(A3)
   Program Desc. : 자동서식변환 용
...
uses
  ...
  AdvGrid, QRPrntr, Variants, VclTee.TeEngine, VclTee.Series, VclTee.TeeProcs, 
  VclTee.Chart, VclTee.DBChart, VclTee.TeeGDIPlus;

type
  TMDN110FM_P01 = class(TForm)
    qr_Icu: TQuickRep;
    QRBand2: TQRBand;
    qrlb_Val0: TQRLabel;
    QRShape111: TQRShape;
    ...
    procedure qr_IcuBeforePrint(Sender: TCustomQuickRep;
      var PrintReport: Boolean);
    procedure FormClose(Sender: TObject; var Action: TCloseAction);
    procedure FormDestroy(Sender: TObject);
    procedure qr_IcuNeedData(Sender: TObject; var MoreData: Boolean);
  ...
  private
    ...
    srcIsLastPage: Integer;         // 마지막 Page Index @ 2012.10.31 LSH
    srcLastDateYn: String;          // 마지막 출력일자 여부 @ 2012.10.31 LSH
  ...
  published
    ...
    property propIsLastPage    : Integer        read  srcIsLastPage write srcIsLastPage; // 2012.10.31 LSH
    property propLastDateYn    : String         read  srcLastDateYn write srcLastDateYn; // 2012.10.31 LSH

  end;


var
  MDN110FM_P01: TMDN110FM_P01;
...
procedure TMDN110FM_P01.qr_IcuBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
var
...
begin
   ...
   //--------------------------------------------------------------------
   // 1-1. Set Header Information "Hospital & Patient"
   //--------------------------------------------------------------------
   ...
   //-------------------------------------------------------------------
   if (qr_Icu.PageNumber = srcIsLastPage) and
      (srcLastDateYn = 'Y') then
   begin
   ...
   //-------------------------------------------------------------------
   // 기록지별 출력양식 항목 분기, 2011.11.01 LSH
   //  - NICU : 체중/두위/복위 항목추가, 2012.01.30 LSH
   //  - SU   : I/O Sum 제거, SU용 차트추가 @ 2012.05.24 LSH
   //-------------------------------------------------------------------
   if (srcNicuFlag = 'Y') then
   begin
      qrlb_ReportName.Caption    := '신생아중환자실 기록지';
      qrlb_ReportName.Font.Size  := 20;

      qrlb_NicuTitle_Wgt.Left    := 716;
      qrlb_NicuTitle_HSize.Left  := 716;
      qrlb_NicuTitle_BSize.Left  := 716;

      qrlb_NicuWgt.Left          := 767;
      qrlb_NicuHSize.Left        := 767;
      qrlb_NicuBSize.Left        := 767;
...
procedure TMDN110FM_P01.qr_IcuNeedData(Sender: TObject;
...
begin
   ...
   begin
     (GetComp(Self, 'qrlb_Head' + inttostr(i)) as TQRLabel).Caption := '';  // <-- A4 서식출력시 주석처리해야 다음 Page Row-Header 나옴.
...
procedure TMDN110FM_P01.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action := caFree;
end;

procedure TMDN110FM_P01.FormDestroy(Sender: TObject);
begin
   MDN110FM_P01 := Nil;
end;
...
initialization
   RegisterClass(TMDN110FM_P01);

finalization
   UnRegisterClass(TMDN110FM_P01);
